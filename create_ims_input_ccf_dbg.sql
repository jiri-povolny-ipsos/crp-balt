...
*
*	Pouští se z fce auto_rel_pub_dbg, protože potřebuje tabulku rel_pub_dbg.crp_all_panel
*	naplněnou správnými midy. Naplní tabulku rel_pub_dbg.crp_ims_input
*
**********************************************************************************************/
DECLARE
...
--*****--		
  ...
  TRUNCATE TABLE rel_pub_dbg.crp_ims_input;
  INSERT INTO rel_pub_dbg.crp_ims_input
  (
    pid,
    mid,
    facepid,
  ...
  SELECT 
    ...
    WHERE 
      a.mid IN(SELECT mid FROM rel_pub_dbg.crp_all_panel)
      AND(a.pnlmotion <> 'D' OR (a.pnlmotion = 'D' AND f2.pos = 1))
      AND v.vacid IN(SELECT DISTINCT ON (a.faceid) a.vacid
                      FROM vai_vac_res a JOIN rel_pub_dbg.crp_all_panel b ON a.mid = b.mid
                      --WHERE a.valid = 1
                      ORDER BY a.faceid,
                               a.vacid DESC
                      )
  ;
	...
	SELECT COUNT(*) INTO i_cnt FROM rel_pub_dbg.crp_ims_input;
--*****--
  RAISE NOTICE '%', CLOCK_TIMESTAMP() || ' Function ims_input_ccf(), num of recs: ' || i_cnt ||'.';
  UPDATE rel_pub_dbg.crp_ims_input  SET pnlvaienv = 'Roadside' WHERE pnlvaienv = 'EA';
  UPDATE rel_pub_dbg.crp_ims_input  SET pnlvaienv = 'Indoor' WHERE pnlvaienv = 'EB';
  -- vysledky z round ------------------------------------------------------------------
  -- VAC
  UPDATE rel_pub_dbg.crp_ims_input a 
  SET 
  	vac_week_all = b.vac
	FROM 
  ...
  ----------------------------------------------
  -- VA
  UPDATE rel_pub_dbg.crp_ims_input  
  SET 
...
--*****--
  RAISE NOTICE '%', CLOCK_TIMESTAMP() || ' Function ims_input_ccf() end.';
	...
	PERFORM rel_pub_dbg.log_msg('INFO', 'fce:create_ims_input_ccf', 'The function completed successfully.');
  
EXCEPTION
WHEN others THEN
  RAISE NOTICE 'exception: %', SQLERRM;
  PERFORM rel_pub_dbg.log_msg('ERROR', 'create_ims_input_ccf', 'Error occured: ' || SQLERRM);
END;